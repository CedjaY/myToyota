		...
		<div class="bs-sidebar">
			<ul id="ul_object" class="nav nav-list bs-sidenav">
				...
				foreach ($eqLogics as $myToyota) {
					...
					elseif ($myToyota->getId() == init('eqLogic_id')) {
						echo '<li class="cursor li_object active" ><a data-eqLogic_id="' .$myToyota->getId(). '" href="index.php?v=d&p=panel&m=' .$pluginName. '&eqLogic_id=' . $myToyota->getId() . '" style="padding: 5px 0px; background-color: rgb(var(--bg-color))"><span>' . $myToyota->getName(). '</span></a></li>';
					}
					else {
						echo '<li class="cursor li_object" ><a data-eqLogic_id="' .$myToyota->getId(). '" href="index.php?v=d&p=panel&m=' .$pluginName. '&eqLogic_id=' . $myToyota->getId() . '" style="padding: 2px 0px;"><span>' . $myToyota->getName(). '</span></a></li>';
					}
				}
				?>
			</ul>
...
<?php include_file('desktop', 'panel', 'js', 'myToyota');?>